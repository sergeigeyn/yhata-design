...
export function StubButton({
  label, icon, description = "Функция в разработке — заглушка для preview",
  className = "", style,
}: {
  label: string;
  ...
  return (
    <>
      <button
        onClick={() => { setShow(true); setTimeout(() => setShow(false), 2500); }}
        className={className}
        style={style}
      >
        {icon}{label}
      ...
      {show && (
        <div className="fixed bottom-24 left-1/2 -translate-x-1/2 z-50 bg-black/90 text-white text-sm px-5 py-3 rounded-2xl shadow-xl max-w-xs text-center">
          <p className="font-medium mb-0.5">{label || "Действие"}</p>
